...
/// Fused input gradient + ReLU backward: dX = (dY · W^T) ⊙ relu_mask.
/// Computes matmul_transB (A * B^T), then applies the ReLU mask
/// from the DESTINATION layer's pre-activation at the output write.
/// This saves one full dispatch by merging the separate relu_backward
/// into the final write of the input gradient matmul.
///
///   A is [M x K], B is [N x K], pre_act is [M x N], out is [M x N].
///   Output element [m, n] = (sum_k A[m*K+k] * B[n*K+k]) * (pre_act[m*N+n] > 0).
///
/// The ReLU mask is applied ONLY at the final write, not in the inner
/// loop, so the tiled matmul performance is unaffected.  The only extra
/// cost is one global memory read per output element.
kernel void matmul_transB_relu(
    device const float* A       [[buffer(0)]],  // [M x K] (grad)
    device const float* B       [[buffer(1)]],  // [N x K] (weights)
    device float*       out     [[buffer(2)]],  // [M x N]
    constant uint& M            [[buffer(3)]],
    constant uint& K            [[buffer(4)]],
    constant uint& N            [[buffer(5)]],
    device const float* pre_act [[buffer(6)]],  // [M x N]
    uint2 gid                   [[thread_position_in_grid]],
    uint2 lid                   [[thread_position_in_threadgroup]])
{
    threadgroup float tileA[TS * TS];
    ...
    for (uint t = 0; t < num_tiles; t++) {
        ...
        // Load B^T tile: B[col, t*TS+lid.y].
        const uint k_b = t * TS + lid.y;
        if (col < N && k_b < K) {
            tileB[lid.y * TS + lid.x] = B[col * K + k_b];
        } else {
    ...
    // Fused ReLU backward: apply mask at the output write.
    // Only one extra global read per output element.
    if (row < M && col < N) {
        const uint idx = row * N + col;
        out[idx] = pre_act[idx] > 0.0f ? sum : 0.0f;
    }
}
