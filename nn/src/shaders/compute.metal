...
// ============================================================================
// qmv_const — constant-cache input, no shared memory
// ============================================================================

/// qmv variant that reads the input vector through Metal's
/// constant address space instead of loading it into threadgroup
/// shared memory.  This eliminates:
///   (1) 8 KB of threadgroup memory per threadgroup,
///   (2) the cooperative load loop (K/512 iterations), and
///   (3) the threadgroup_barrier before computation.
///
/// The constant cache on Apple Silicon is 64 KB per shader core,
/// so the 8 KB input vector (K <= 2048) fits entirely.  All
/// threadgroups reading the same input get hardware-level
/// broadcast — no redundant loads.
///
/// Zero threadgroup memory means the GPU can schedule many more
/// threadgroups per execution unit (occupancy limited only by
/// registers, not shared memory).
///
/// Processes 2 rows per simdgroup (32 rows/tg, same as sm2).
/// Only valid for K <= 2048, single scale group per lane.
///
/// Dispatch: threadgroups = ceil(M / 32), threads = 512.
kernel void qmv_const(
    device const uint8_t* packed_bits  [[buffer(0)]],
    device const half*    scales       [[buffer(1)]],
...
// ============================================================================
// f16-input kernel variants
// ============================================================================
//
// The following kernels are identical to their f32-input
// counterparts except the activation input vector is half*.
// Every input read is widened to float immediately so all
// accumulation remains in f32 — critical for precision.
// Output buffers stay f32.
// ============================================================================

// ============================================================================
// qmv_f16in — half-precision input, device address space
// ============================================================================

/// f16 input variant of qmv.  Reads half-precision activation
/// vector and widens each element to float before accumulation.
/// All arithmetic stays in f32 for precision.  Output is f32.
///
/// Dispatch: threadgroups = ceil(M / 2), threads_per_group = 64.
kernel void qmv_f16in(
    device const uint8_t* packed_bits  [[buffer(0)]],
    device const half*    scales       [[buffer(1)]],
...
// ============================================================================
// qmv_const_f16in — constant-cache input, half-precision
// ============================================================================

/// f16 input variant of qmv_const.  Reads the activation vector
/// as half through Metal's constant address space — the input
/// vector is now 4 KB instead of 8 KB for K=2048, fitting even
/// more comfortably in the 64 KB constant cache.  Every element
/// is widened to float before accumulation.  Output is f32.
///
/// Dispatch: threadgroups = ceil(M / 32), threads = 512.
kernel void qmv_const_f16in(
    device const uint8_t* packed_bits  [[buffer(0)]],
    device const half*    scales       [[buffer(1)]],
...
// ============================================================================
// qmv_fused_pair_const_f16in — fused pair, constant-cache, f16
// ============================================================================

/// f16 input variant of qmv_fused_pair_const.  Reads the
/// activation vector as half through the constant address space.
/// Every element is widened to float before accumulation.
/// Processes 2 rows from BOTH weight matrices per simdgroup.
/// Output is f32.
///
/// Dispatch: threadgroups = ceil(M / 32), threads = 512.
kernel void qmv_fused_pair_const_f16in(
    device const uint8_t* packed_a  [[buffer(0)]],
    device const half*    scales_a  [[buffer(1)]],
    constant half*        input     [[buffer(2)]],
    device float*         output_a  [[buffer(3)]],
...
// ============================================================================
// qmv_const_multigroup_f16in — constant-cache, multi-group, f16
// ============================================================================

/// f16 input variant of qmv_const_multigroup.  Reads the
/// activation vector as half through the constant address space.
/// Every element is widened to float before accumulation.
/// Handles K > 2048 where lanes span multiple scale groups.
/// Input accessed via constant cache (64 KB limit → K <= 32768
/// at half precision).  Output is f32.
///
/// Dispatch: threadgroups = ceil(M / 32), threads = 512.
kernel void qmv_const_multigroup_f16in(
    device const uint8_t* packed_bits  [[buffer(0)]],
    device const half*    scales       [[buffer(1)]],
    constant half*        input        [[buffer(2)]],
    device float*         output       [[buffer(3)]],
...
// ============================================================================
// f16-input/output kernel variants
// ============================================================================
//
// The following kernels are identical to their f16in counterparts
// except the output buffer is also half*.  All internal
// accumulation remains in f32 — only the final write narrows
// to half via half(acc).  This halves output memory bandwidth
// and allows downstream kernels to stay in f16 without a
// format conversion pass.
// ============================================================================

// ============================================================================
// qmv_f16io — half-precision input and output, device address space
// ============================================================================

/// f16 input and output variant of qmv.  Reads half-precision
/// activation vector, accumulates in f32, writes half-precision
/// output.
///
/// Dispatch: threadgroups = ceil(M / 2), threads_per_group = 64.
kernel void qmv_f16io(
    device const uint8_t* packed_bits  [[buffer(0)]],
    device const half*    scales       [[buffer(1)]],
    device const half*    input        [[buffer(2)]],
    ...
    uint                  tid_in_tg    [[thread_index_in_threadgroup]])
{
    ...
    // Lane 0 writes the final result narrowed to f16.
    if (lane == 0) {
        output[row] = half(accum);
    }
}
...
/// f16 input and output variant of qmv_const.  Reads
/// half-precision activation through Metal's constant address
/// space, accumulates in f32, writes half-precision output.
///
/// Dispatch: threadgroups = ceil(M / 32), threads = 512.
kernel void qmv_const_f16io(
    device const uint8_t* packed_bits  [[buffer(0)]],
    device const half*    scales       [[buffer(1)]],
    constant half*        input        [[buffer(2)]],
    device half*          output       [[buffer(3)]],
...
// ============================================================================
// qmv_fused_pair_const_f16io — fused pair, constant-cache, f16 I/O
// ============================================================================

/// f16 input and output variant of qmv_fused_pair_const.  Reads
/// the activation vector as half through the constant address
/// space, accumulates in f32, writes half-precision output.
/// Processes 2 rows from BOTH weight matrices per simdgroup.
///
/// Dispatch: threadgroups = ceil(M / 32), threads = 512.
kernel void qmv_fused_pair_const_f16io(
    device const uint8_t* packed_a  [[buffer(0)]],
    device const half*    scales_a  [[buffer(1)]],
    constant half*        input     [[buffer(2)]],
...
// ============================================================================
// qmv_const_multigroup_f16io — constant-cache, multi-group, f16 I/O
// ============================================================================

/// f16 input and output variant of qmv_const_multigroup.  Reads
/// the activation vector as half through the constant address
/// space, accumulates in f32, writes half-precision output.
/// Handles K > 2048 where lanes span multiple scale groups.
/// Input accessed via constant cache (64 KB limit → K <= 32768
...
///
/// Dispatch: threadgroups = ceil(M / 32), threads = 512.
kernel void qmv_const_multigroup_f16io(
    device const uint8_t* packed_bits  [[buffer(0)]],
    device const half*    scales       [[buffer(1)]],
    constant half*        input        [[buffer(2)]],
    device half*          output       [[buffer(3)]],
