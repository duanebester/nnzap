...
fn convertScalesAffineToSymmetric(
    source: []const f16,
    target: []f16,
) void {
...
fn convertF32ToF16(source: []const f32, target: []f16) void {
    std.debug.assert(source.len == target.len);
    std.debug.assert(source.len > 0);
    for (source, target) |s, *t| {
        t.* = @floatCast(s);
...
test "Model allocates all buffers" {
    ...
    var model: Model(Config) = undefined;
    try model.init(device_obj);
    defer model.deinit();

    // Embedding.
    try std.testing.expectEqual(
        Config.vocab_size * Config.hidden_size,
        model.embedding.packed_count,
    );
    try std.testing.expectEqual(
        @as(u32, 128),
        model.embedding.group_size,
    );
    ...
    for (0..Config.num_layers) |i| {
        try std.testing.expectEqual(
            ...
            model.q_proj[i].packed_count,
        );
        try std.testing.expectEqual(
            Config.hidden_size * Config.kv_dim,
            model.k_proj[i].packed_count,
        );
        try std.testing.expectEqual(
            Config.hidden_size * Config.kv_dim,
            model.v_proj[i].packed_count,
        );
        try std.testing.expectEqual(
            Config.query_dim * Config.hidden_size,
            model.o_proj[i].packed_count,
        );
        try std.testing.expectEqual(
            Config.hidden_size * Config.intermediate_size,
            model.gate_proj[i].packed_count,
        );
        try std.testing.expectEqual(
            Config.hidden_size * Config.intermediate_size,
            model.up_proj[i].packed_count,
        );
        try std.testing.expectEqual(
            Config.intermediate_size * Config.hidden_size,
            model.down_proj[i].packed_count,
        );
    }
    ...
    for (0..Config.num_layers) |i| {
        try std.testing.expectEqual(
            Config.hidden_size,
            model.attn_norm[i].len,
        );
        try std.testing.expectEqual(
            Config.hidden_size,
            model.ffn_norm[i].len,
        );
        try std.testing.expectEqual(
            Config.head_dim,
            model.q_norm[i].len,
...
test "forwardBlockArgs returns correct handles" {
    ...
    const args0 = model.forwardBlockArgs(0, 0, 1);
    const args1 = model.forwardBlockArgs(1, 0, 1);
