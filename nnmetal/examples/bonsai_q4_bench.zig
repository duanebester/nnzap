//! Bonsai 1.7B Q4 inference benchmark — structured throughput measurement.
//!
//! Measures prefill and decode throughput, per-token latency percentiles,
//! and writes structured JSON results to stdout.
//!
//! This is the Q4 MLX (4-bit affine) counterpart to bonsai_bench.zig.
//! The only differences from the Q1 benchmark are:
//!   - Config: Bonsai1_7B_Q4 (vocab_size=151936, group_size=64)
//!   - Specialized pipelines: specialized_q4mv instead of specialized_qmv
//!   - Default model path: ~/models/qwen3-1.7b-q4
//!   - JSON model field: "Qwen3-1.7B-Q4"
//!
//! Usage:
//!   zig build run-bonsai-q4-bench -- [model_dir]
//!
//! The model directory defaults to data/qwen3-1.7b-q4 relative to
//! the nnmetal project root, falling back to ~/models/qwen3-1.7b-q4.
...
pub fn main() !void {
    ...
    std.debug.print(
        "\n\x1b[1m[nnmetal] Qwen3 1.7B Q4 Benchmark\x1b[0m\n" ++
            "model:          {s}\n" ++
...
fn formatEpochUtc(
    ...
    // Civil date from day count (Howard Hinnant's
    // chrono algorithm, public domain).
...
/// Resolve the model directory from CLI args, then try
/// data/qwen3-1.7b-q4 relative to the executable, then
